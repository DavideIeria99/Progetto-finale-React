...
export default function Search() {
  ...
  const triggerSearch = () => {
    setLoading(true);
    setGames(null)
    fetch(`${import.meta.env.VITE_RAWG_API_URL}games?&key=${import.meta.env.VITE_RAWG_API_KEY}&page_size=${page_size}&search=${searched}&search_precise=true&&ordering=+rating `,)
  ...
  return (
    ...
    <div className="flex min-h-screen px-6">
      <div className=" flex w-1/8 me-3 flex-col">
        <div>
          <input
            type="text"
            className="border-2 border-slate-300 me-1 border-accent  bg-transparent  text-slate-700   dark:text-white"
            placeholder="search by name..."
            value={searched}
            onChange={(e) => setSearched(e.target.value)} />
          <button onClick={triggerSearch} className="bg-slate-800 p-1 rounded-lg">Search</button>
        </div>
